...
def run_one_max(initial_state, vector_dimension):
    ...
    start = datetime.now()
    ga_results, ga_fitness =  mlrose.genetic_alg(problem, max_attempts=100, max_iters=1000,mutation_prob=0.5);
    end = datetime.now()
    results['GA'] = [ga_fitness, (end-start).total_seconds()]

    print("Running MIMIC")
    start = datetime.now()
    mimic_results, mimic_fitness = mlrose.mimic(problem, max_attempts=100, max_iters=1000,keep_pct=.5);
    end = datetime.now()
